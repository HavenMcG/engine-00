...
void Renderer::draw_mesh(const Mesh& mesh, const Material& material, Shader* shader, MeshManager& mm, TextureManager& tl) {
	// check loaded here?
	...
	for (int i = 0; i < material.diffuses.size() && i <= 1; ++i, ++total_textures) { // temp
		...
		tl.id(material.diffuses[0].name, id);
	...
	for (int i = 0; i < material.speculars.size() && i <= 1; ++i, ++total_textures) { // temp
		glActiveTexture(GL_TEXTURE0 + total_textures);
		tl.id(material.speculars[0].name, id);
